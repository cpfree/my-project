...
let vueContent = new Vue({
    ...
    methods: {
        ...
        thisCall : function(type, data) {
            ...
            if (type === 'edit') {
                ...
                layer.confirm('确定删除记录吗？', {
                    ...
                    layer.close(index);
                    $.sendPostRequest('sysDictType/remove', {nameArr : [data]}, (data, status)=>{
        ...
        deleteSelection : function(){
            ...
            layer.confirm('确定删除记录吗？', {
                ...
                layer.close(index);
                let params = {
                    nameArr : _this.sysDictTypeGrid.multipleSelection.map(it=>it.name)
                };
    ...
    components: {
        ...
        cOperate : {
            ...
            methods: {
                editRecord : function () {
                    this.$emit('callmethod', 'edit', this.row.name);
                },
                deleteRecord : function () {
                    this.$emit('callmethod', 'delete', this.row.name);
                }
            }
        }
    },
